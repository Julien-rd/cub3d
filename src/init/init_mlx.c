...
/*   Updated: 2026/04/15 15:56:39 by vmanuyko         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	open_window(t_user *user)
{
	user->mlx_win = mlx_new_window(user->mlx, SCREEN_WIDTH, SCREEN_HEIGHT,
			"cub3D");
	if (!user->mlx_win)
		exit_game(user, ERROR, "mlx_new_window failed");
	user->tex.img.img = mlx_new_image(user->mlx, SCREEN_WIDTH, SCREEN_HEIGHT);
	if (!user->tex.img.img)
		exit_game(user, ERROR, "mlx_new_img failed");
	...
	if (!user->tex.img.data)
		exit_game(user, ERROR, "mlx_get_data_addr failed");
}
...
static void	load_textures(t_user *user)
{
	...
	if (!user->tex.n.img)
		exit_game(user, ERROR, "Failed to load north texture");
	...
	if (!user->tex.s.img)
		exit_game(user, ERROR, "Failed to load south texture");
	...
	if (!user->tex.w.img)
		exit_game(user, ERROR, "Failed to load west texture");
	...
	if (!user->tex.e.img)
		exit_game(user, ERROR, "Failed to load east texture");
...
void	init_graphics(t_user *user)
{
	user->mlx = mlx_init();
	if (!user->mlx)
		exit_game(user, ERROR, "mlx_init failed");
	load_textures(user);
	open_window(user);
}
